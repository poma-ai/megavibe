...
**Choosing which reviewers run.** Megavibe reviews in two tiers. Every round gets one independent reader — Codex, or the Claude `reviewer` subagent on a machine without Codex. The round that gates the ship, and anything critical (credentials, destructive paths, public-facing work, the protocol itself), gets both in parallel. The reason is cost: Codex draws on its own plan, while the subagent spends ~180-200K tokens of the same Claude subscription your session is using, so running both on every intermediate round buys a second opinion on work that is still moving. Gemini is their fallback, not a third peer — it reviews only when Codex fails on the day, and always on `--pro`. If you want something else:
