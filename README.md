...
**`kubectl` never asks a human to log in.** Many orgs enforce a re-auth policy — Google Cloud RAPT, AWS SSO, an Okta-fronted OIDC provider — that refuses to refresh credentials outside an interactive terminal, so `kubectl` in an agent session dies hourly with something like `Reauthentication failed. cannot prompt during non-interactive execution`. The usual answer is a small keeper script that mints a short-lived bearer token to a file, and it works right up until the agent forgets the keeper exists and tells you to go log in instead. (Measured in one real setup before this hook: 19 such asks across 18 transcripts in 9 projects, with the keeper one command away the whole time.) Point `kube-token.sh` at your keeper and it stops being something anyone has to remember: on any Bash command with `kubectl` in command position, it runs the keeper when the token file is missing or stale, then rewrites the command to carry `--token="$(cat …)"`. Configure it in `~/.megavibe/personal/kube-token.conf` (or the same three environment variables) with `KUBE_TOKEN_FILE`, `KUBE_TOKEN_KEEPER` and optionally `KUBE_TOKEN_MAX_AGE` (default 50 min); with none of them set the hook is inert, so it costs nothing if you don't use Kubernetes. The token goes in as a command substitution, never a shell variable — zsh does not word-split, and a token held in a variable leaks into transcripts — and it never enters the model's context. Only `kubectl` in command position is touched: a command that already passes `--token`, a quoted mention, and a literal `grep kubectl` are all left alone.
