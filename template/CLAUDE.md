...
4. **Independent review by default, in two tiers.** Every important result, change, or product gets independent review before it ships, without the user asking. Important = new/changed code beyond trivial mechanical edits, documents or plans the user will rely on, analyses that drive decisions, anything user-facing. **Every round gets one reviewer: Codex when it is switched on and available** (`~/.megavibe/scripts/codex-review.sh --as-reviewer`; it forces `--sandbox read-only` and refuses the write escapes, so a reviewer can never acquire write access). Otherwise — switched off, not installed, or the call itself **failed** on quota or timeout — **the `reviewer` subagent takes the round.** Gemini may join it, never stand alone: a round whose only reader is Gemini is weaker than the weakest configuration this protocol documents. The floor is one reviewer that actually ran, never zero, and a skipped or failed call is not a completed review. Leave `--model`/`--effort` off a review; the user's own `~/.codex/config.toml` picks those. **The round that gates the ship, and anything critical, runs every reviewer that is both switched on and available, in parallel** — the **`reviewer` subagent** (`.claude/agents/reviewer.md`, Opus, fresh context, runs things), Codex, and Gemini too where the user pinned it as a peer. Critical work is as `delegation.md` defines it — credentials, destructive paths, anything public or user-facing, the protocol and templates, and anything the user names. **The gate is prospective:** before merging, deploying, publishing, or declaring an important result done, the final candidate must have had a full-set review. An intermediate review cannot authorise shipping; if a round turns out to be the last, run the missing reviewers on the final state before proceeding, and the one-re-review-round limit does not waive this. Agents register at session start, so in a session that was already open when the file landed `subagent_type: reviewer` is "not found": then use `general-purpose` with `model: opus` and paste the text of `.claude/agents/reviewer.md` as the brief — never skip it on a ship round. `/megavibe-restart` registers it. **Gemini is Codex's fallback, not a peer.** When Codex fails on the day you MAY add `gemini-review.sh --as-reviewer --fallback --pro` alongside the subagent; say in the synthesis that it stood in. Always `--pro`; never the MCP tool or `gemini -p` for reviews, which truncate or stall on 3.x thinking. Mechanics in `delegation.md`. Run every reviewer in the round with the same files and the same prompt; synthesize; act on findings before declaring done. **One round per shippable unit is the baseline**; extra intermediate rounds are for work still in flight, and at most one re-review round after fixes when a reviewer said "do not ship" — then ship and report what remains. If ambiguous, risky, or repeatedly corrected: ask each reviewer for the neutral, devil's-advocate and optimistic cases. Reviewers must be told to read the document that states the standard (README/CLAUDE.md) before the diff. Exceptions: trivial/mechanical edits, deliverables that are themselves reviews (no recursion), and urgent hotfixes (ship, then review immediately after). **A user can switch the EXTERNAL reviewers off** with `MEGAVIBE_REVIEWERS` (`megavibe reviewers set`); the `reviewer` subagent never can be. Unset is the default above; `all` makes Gemini eligible as a peer. A reviewer outside the set exits 4 under `--as-reviewer` — a setting, not an outage, and not a failed review — and it gates the reviewer ROLE only, so `/rehydrate`, summaries and large-context work reach these same backends regardless. Full detail — tiering, costs, precedence, the committed-settings rule, the fallback mechanism: **`.claude/rules/delegation.md`**.
...
6. **Work from current code.** At session start, and before you commit, open/update a PR, or push: `git fetch` and measure the branch relationship (`git status -sb`, or `git rev-list --count HEAD..@{u}` / `@{u}..HEAD` for deterministic ahead/behind counts) — multiple sessions and machines push to these repos, so a checkout from earlier today may already be stale. **Never flag stale state without having fetched and measured first.** When the branch is strictly *behind* its upstream (zero commits ahead — a pure fast-forward): if the working tree is clean, just do it — `git merge --ff-only @{u}` — and report in one line what moved (undo is `git reset --hard ORIG_HEAD`); do this at session start too, BEFORE editing, so work never starts on a stale base. If the tree is dirty, report the counts and offer the fast-forward instead of stashing silently — stash-pop over a moved base is where conflicts live; if the user accepts, stash per rule 5, and if popping afterwards conflicts: stop, don't commit, ask. Only **stop and surface** when the branch has actually *diverged* (ahead AND behind), or the comparison target is `main` rather than the upstream — don't implicitly `git pull` or create a merge commit there; rebase only when the repo's workflow calls for it. If fetch fails, there's no upstream, `HEAD` is detached, a rebase/merge is in progress, the clone is shallow, or the branch isn't clearly comparable, don't guess — report the state and wait.

7. **A record must say how it was established, and where the thing durably lives.** Compaction strips the tool output that would contradict a claim, so every line you wrote comes back as settled fact in the same typeface — a verified `git` reading and an unchecked guess become indistinguishable. Two cheap markers keep them apart (both learned on 2026-09-18, when a pre-compact record carried an inference that was already false when written, and the post-compaction session repeated it to the user twice as fact):
   - **Provenance.** Tag load-bearing claims with how and when you checked: `v1.1.0 tag on d65d186c [git tag --points-at, 18:09Z]` versus `[inferred, not re-checked]`. After a compaction, re-verify the lines marked unverified BEFORE repeating them — a check that stays short only because the markers exist. Markers are not a substitute for currency: a claim verified last week is a historical observation, so re-read anything the pre-compact hook flags as aged, and anything mutable, against the repo before presenting it as current. Recording a release as a marker the machine can check — `v1.1.0 [git tag --points-at, 18:09Z]` — beats prose a later reader has to interpret.
   - **Canonical destination.** Record where a deliverable durably lives — the release, the PR, the repo path — never only the working copy you produced. Files on the user's Desktop are an outbox (reread, pasted into Slack, forwarded), routinely moved or deleted once the content graduates somewhere canonical; a note naming only that copy ages into a dangling pointer, and its later absence reads as evidence the record lied. The same applies to a task register pointing at a scratch file instead of the system of record.

   Corollary for the user-facing answer: when asked whether things are ready, saved, or safe, say **what you checked and what you did not**. A narrow check reported as a broad yes is the failure these markers exist to catch.
...
**Pre-compact** (PreCompact hook): A hook stamps the compaction summary with the current `.agent/` file status and staleness (tool calls since last write), adds a register-currency warning when `TASKS.md` section 0 carries a date stamp older than seven days (silent unless the project uses that section and stamps it; `MEGAVIBE_REGISTER_MAX_AGE_DAYS` tunes the age), echoes the same report to stderr so the user sees it, and writes `.agent/LOGS/pre-compact-alert.${SID}.md` as a durable audit trail. This tells post-compaction Claude whether files might be stale.
...
**Context watcher** (default ON; opt out with `MEGAVIBE_WATCHER=0`): an out-of-band daemon that flushes `.agent/` files on a trickle (every 5 min) directly from the transcript, replacing in-session nudges. Decisions are staged for human review; narrative/lessons/tasks auto-apply under flock. When the watcher is alive, in-session tier nudges suppress. Full detail: **`README-watcher.md`**.
...
- Important results/changes/products: independent review per non-negotiable 4 — Codex where it is switched on and available, otherwise the `reviewer` subagent, plus the subagent on the ship round and on anything critical — by default, not on request.
...
After significant docs or code edits that change the project's documentation surface, run **`/doc-review`** — sends the project MD set (`CLAUDE.md` + every `README*.md`) to whichever reviewers of non-negotiable 4 are switched on (Claude `reviewer` subagent + Codex by default) **in parallel** for an independent challenge: drift, contradictions, dead pointers, bloat. Synthesize, then act on findings. Not every session — a regular exercise after material doc changes.
...
The Claude subagent (`.claude/agents/summarizer.md`) sits between them for non-review work: no key, always available, and the best output of the three — but it spends the same subscription quota this session runs on, so it is second, not first. Never retry a failed MCP call more than once — switch to the next fallback immediately.
...
**Respect execution mode.** Plan mode and the Task tools (`EnterPlanMode`, `TaskCreate`, `TaskUpdate`) are no longer blocked — the old unbypassable-prompt bug was fixed in Claude Code 2.1.9. But `ExitPlanMode` still waits for human approval by design, so when the user asks for autonomous/unattended execution (or says "do NOT switch to plan mode"), don't *proactively* enter plan mode or spawn a task list — Claude would stall at the approval gate. Just execute directly, reporting progress via text output. Plans you do make persist to `.agent/PLANS/` (native `plansDirectory` setting), so they stay in the repo dir, not the user-global `~/.claude/plans/`.
...
**Never answer an auth failure with "run `gcloud auth login`".** `cloud-token.sh` rewrites `kubectl` and `gcloud` in command position to carry a freshly minted token, so a reauth wall is the hook's job, not the user's — it is inert until `~/.megavibe/personal/cloud-token.conf` names a token file and a keeper. `gcloud auth`, `components`, `config` and `init` are deliberately never rewritten (they reject the flag). If a command still fails on auth, report that plainly instead of sending the user to an interactive login.
...
**Long-running processes.** Don't disown servers, watchers, or polling loops to launchd — no `nohup`, no bare `&` from a shell that's about to exit, no `disown`. Use Bash `run_in_background`, foreground in a terminal the user is watching, or a named tmux session. Anything else and the user finds the process parented to PID 1 a week later. Bind dev servers to `127.0.0.1` by default — uvicorn, `python -m http.server`, vite, next dev all default to `0.0.0.0`, exposing internal services on the LAN; change it explicitly. Polling loops against authenticated APIs (kubectl, gcloud, GitHub, k8s) **must exit on auth failure** rather than retry — one watchdog with no fail-fast generated 1.2M gcloud log files in 14 days. When you start a long-running process, report PID/session, port, how to find it, and how to kill it. **A background task nobody is watching is a task that can crashloop forever.** The harness notifies on completion only; a hung or restarting task never completes, so silence looks exactly like progress. Never end a turn "waiting" on a `run_in_background` Bash or an async Agent: arm a `Monitor` heartbeat on its output file (every ≤5 min: byte or entry delta plus tail) the moment you start it, read the output on every beat, and judge `+0` against what the task is — a build at `+0` is hung, an idle server at `+0` is fine, repeating lines are a crashloop — then act. `watch-background.sh` tells you to arm it at start and blocks a turn from ending on an unchecked task older than the interval (default 5 min, `MEGAVIBE_BG_CHECK_SECS`) — the net, not the mechanism. Full rules: **`.claude/rules/process-discipline.md`**.

**Estimate in AI-pace units, not human-engineer units.** Default to turns, tool-calls, or "this session". Never give wall-clock days/weeks for work Claude is doing — those numbers are trained on human-engineer schedules and are wrong by 1–2 orders of magnitude for AI execution, causing the user to defer work that's actually trivial. Wall-clock figures are allowed only when the user explicitly asks AND the answer is framed as "if a human were doing this". When unsure, just start — progress is cheaper than the estimate.

**Don't stall mid-task.** Never ask "should I continue?", "good stopping point?", or "shall we call it a day?" while work is in flight. Push through to done — the user calls the end of a session, not you. Legitimate reasons to pause: genuine ambiguity in the spec, a destructive/irreversible action needing confirmation, missing credentials or a broken tool, or the current plan is failing and needs re-assessment (see Workflow → Implement). Tiredness, length, "this is a lot", or "we've made good progress" are NOT blockers. When you do hit a real blocker, name it explicitly ("blocked: need X") instead of offering an off-ramp.
