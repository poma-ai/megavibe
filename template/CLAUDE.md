...
Claude Code is the executor and orchestrator. **Codex is the primary subcontractor** — reviews, re-hydration, summaries, large context — reached through `~/.megavibe/scripts/`. Gemini is the fallback behind it, and no longer a peer reviewer: measured over one day of paired reviews it shipped real blockers and invented findings, while Codex, which runs the code, did not (`.claude/rules/delegation.md` has the numbers). Codex has no MCP server at all (codex-cli 0.154.0 removed it); Gemini's MCP survives for short interactive questions only, since it truncates on 3.x thinking. Playwright handles UI automation. **Megavibe works with only a Claude Code subscription** — external backends improve quality but are never required.
...
4. **Independent review by default — two reviewers, three on request.** Every important result, change, or product gets independent review before it ships, without the user asking. Important = new/changed code beyond trivial mechanical edits, documents or plans the user will rely on, analyses that drive decisions, anything user-facing. Always: the **`reviewer` subagent** (`.claude/agents/reviewer.md`, Opus, fresh context, runs things — same Claude subscription). Agents register at session start, so in a session that was already open when the file landed `subagent_type: reviewer` is "not found": then use `general-purpose` with `model: opus` and paste the text of `.claude/agents/reviewer.md` as the brief — never skip it. `/megavibe-restart` registers it. Plus, whenever it is up: **Codex** (`~/.megavibe/scripts/codex-review.sh --as-reviewer`; it forces `--sandbox read-only` and refuses the write escapes, so a reviewer can never acquire write access — why, in `.claude/rules/delegation.md`). Leave `--model`/`--effort` off a review: the user's own `~/.codex/config.toml` picks those, and a review is the one job where thinking earns its cost. **Gemini is the fallback, not a third peer.** When Codex fails on the day — quota, outage, timeout — re-run that review as `gemini-review.sh --as-reviewer --fallback --pro` and say in the synthesis that it stood in. Without `--fallback` it exits 4 in the default set. Always `--pro`: the cheap tier returned zero-finding praise on code with a P1 in it. Never the MCP tool or `gemini -p` for reviews — they truncate or stall on 3.x thinking. Run every switched-on reviewer in parallel with the same files and the same prompt; synthesize; act on findings before declaring done. One round per shippable unit, plus at most one re-review round after fixes when any reviewer said "do not ship" — then ship and report what remains. If ambiguous, risky, or repeatedly corrected: ask each reviewer for the neutral, devil's-advocate and optimistic cases. Reviewers must be told to read the document that states the standard (README/CLAUDE.md) before the diff. Exceptions: trivial/mechanical edits, deliverables that are themselves reviews (no recursion), and urgent hotfixes (ship, then review immediately after). **A user can switch reviewers off**: `MEGAVIBE_REVIEWERS` is an allow-list of the EXTERNAL reviewers (`gemini`, `codex`). Unset = the default: Codex when `codex exec` works here, else Gemini. `all` restores all three in parallel; `reviewer codex` refuses Gemini even as a fallback. The `reviewer` subagent is always in the set and cannot be switched off — it is the floor this rule rests on, so "never skip it" above stands unconditionally, set with `megavibe reviewers set` or by hand in the `env` block of `~/.claude/settings.json` or a project's uncommitted `.claude/settings.local.json`. A project's COMMITTED `.claude/settings.json` is read too but may only *add* a reviewer, never remove one — it arrives with a clone, and a repo that can switch off the reviewers reading its own code is a hole. The session-start table has a Reviewers row; a reviewer left out of the set exits 4 under `--as-reviewer`, so do not ask it to review and do not treat its absence as a failed review. It gates the reviewer role only — **without `--as-reviewer` these same scripts stay megavibe's general Gemini/Codex path**, so `/rehydrate`, summaries and large-context work are unaffected by the setting.
...
- Important results/changes/products: independent review per non-negotiable 4 — the `reviewer` subagent plus Codex, with Gemini as Codex's fallback — by default, not on request.
...
**Gemini is available iff `$GEMINI_API_KEY` is set** (a key from a billed project — Google-account OAuth was retired June 2026, and the free tier is 20 req/day and trains on prompts). It is the fallback for both roles, so a missing key costs nothing while Codex is up. The `mcp__gemini-cli__ping` result only gates the `ask-gemini` MCP tool, which is for short interactive questions; a failed ping does not make Gemini unavailable.
...
The Claude subagent (`.claude/agents/summarizer.md`) sits between them for non-review work: better output than either on a context digest, no key, always available — but it spends the same subscription quota this session runs on (125K tokens on a 196 KB input, measured), so it is the second choice, not the first. Never retry a failed MCP call more than once — switch to the next fallback immediately.
