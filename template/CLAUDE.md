...
**Proactive nudge** (before compaction triggers): A hook measures transcript token usage and nudges at three escalating tiers — 🟡 50% / 🟠 75% / 🔴 90% of the model's effective auto-compact threshold (e.g. ~250K / 375K / 450K on 1M-context models, ~83K / 125K / 150K on 200K-native). The harness threshold is set by megavibe's launcher via `CLAUDE_CODE_AUTO_COMPACT_WINDOW=533000` and clamps to model-native context; override via env or `autoCompactWindow` in `.claude/settings.json`. Each tier fires at most once; the counter resets after an actual compaction. At every tier: **flush all pending context to `.agent/` files first**, then run `/compact`. Post-compaction recovery only has what's on disk.
