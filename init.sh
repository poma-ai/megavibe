...
HOOKS_MISSING=0
for hook in log-tool-event.sh block-dangerous-bash.sh block-plan-mode.sh block-stray-working-context.sh nudge-native-tools.sh nudge-quiet-bash.sh after-edit.sh on-compact.sh on-pre-compact.sh on-session-start.sh on-session-end.sh start-context-watcher.sh augment-search.sh resize-image.sh read-delta.sh truncate-verbose-bash.sh; do
